...
func _process(delta: float) -> void:
	...
	weapon.rotation = weapon.global_position.direction_to(get_global_mouse_position()).angle()
...
func movement_z(delta: float) -> void:
	...
	set_collision_mask_value(1, position_z > -TILE_HEIGHT)
	set_collision_mask_value(2, position_z > -TILE_HEIGHT * 2)
	set_collision_mask_value(3, position_z > -TILE_HEIGHT * 3)
	
	var found := false
	for body in world_area.get_overlapping_bodies():
		if body is WorldLayer:
			if body.height * TILE_HEIGHT >= floor_height:
				floor_height = body.height * TILE_HEIGHT
				found = true
